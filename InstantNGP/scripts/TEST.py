...
def icp(src_tm: "<class 'trimesh'>", dst_tm: "<class 'trimesh'>",
    ...
    vec = np.column_stack((src_tm.vertices, src_tm.vertices + (src_tm.vertex_normals * src_tm.scale * 0.5)))
    path = trimesh.load_path(vec.reshape((-1,2,3)))
    trimesh.Scene([src_tm, path]).show(smooth=False)
    ...
    for i in range(max_iterations):
        ...
        # rgbs = [(1- i, i, 0) for i in anglesFinal] # rgbs = [(1 - i, i, 0) for i in anglesFinal]
        #
