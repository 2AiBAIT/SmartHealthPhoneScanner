...
def parse_args():
	...
	parser.add_argument("--save_mesh", default="/home/hcorreia/PycharmProjects/instant-ngp/Api/Database",
	...
	parser.add_argument("--epochs", default=9000)
	parser.add_argument("--username", default="helena")
...
if __name__ == "__main__":
	...
	datatime = datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ")
	savemesh = args.save_mesh + '/' + args.username + '/' + datatime + '.ply'

	res = args.marching_cubes_res or 256
	print(
		f"Generating mesh via marching cubes and saving to {savemesh}. Resolution=[{res},{res},{res}]")
	testbed.compute_and_save_marching_cubes_mesh(savemesh, [res, res, res])

	ms = pymeshlab.MeshSet()
	ms.load_new_mesh(savemesh)
	...
	ms.save_current_mesh(savemesh, binary=False)
	...
	#
	# 	file_path = "/home/hcorreia/PycharmProjects/instant-ngp/data/test.obj"
	# 	mesh = o3d.io.read_triangle_mesh(file_path)
	# 	mesh_out = mesh.filter_smooth_laplacian(number_of_iterations=10)
	# 	voxel_size = max(mesh_out.get_max_bound() - mesh_out.get_min_bound()) / 128
	# 	print(f'voxel_size = {voxel_size:e}')
	# 	mesh_out = mesh_out.simplify_vertex_clustering(voxel_size=voxel_size,
...
# triangles_to_remove = cluster_n_triangles[triangle_clusters] < 10000
# mesh_0.remove_triangles_by_mask(triangles_to_remove)
# print(f'Output mesh has {len(mesh_0.vertices)} vertices and {len(mesh_0.triangles)} triangles')
# o3d.io.write_triangle_mesh(file_path, mesh_0)
